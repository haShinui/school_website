<template>
  <div id="app">
    ...
    <Menubar :model="menuItems">
      ...
      <!-- Profile icon on the right side with a Menu Popup -->
      <template #end>
        <Button type="button" icon="pi pi-ellipsis-v" @click="toggleMenu" aria-haspopup="true" aria-controls="overlay_menu" />
        <Menu ref="menu" id="overlay_menu" :model="menuItemsProfile" :popup="true"  />
      </template>
    </Menubar>
...
// Menu items for profile, home, and settings with separator
const menuItemsProfile = ref([
  ...
  { separator: true }, // Separator above Settings
  {
    label: 'Settings',
    icon: 'pi pi-cog',
    command: () => {
...
const menuItems = computed(() => {
  ...
  // Example authenticated user logic
  if (store.getters.isAuthenticated) {
    items.push({ label: 'Dashboard', icon: 'pi pi-fw pi-cog', route: '/dashboard' });
  }
...
li.p-menu-separator {
    border-top: 3px solid #0f0fb2 !important;
}
</style>
